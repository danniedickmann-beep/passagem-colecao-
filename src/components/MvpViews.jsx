...
export function TelaoView({ pieces, collectionOptions = [], criticalityOptions = [], problemTypeOptions, selectedId, setSelectedId, onCreatePending, saving }) {
  ...
  return <Page title="Passagem de Coleção" kicker="Apresentação da coleção" description="Selecione a coleção e a criticidade para conduzir a apresentação dos artigos correspondentes.">
    ...
    {presentationMode && <div className="presentation-stage" role="dialog" aria-modal="true" aria-label={`Apresentação do artigo ${piece.artigo}`}><header><div><small>NEXUS · MODO APRESENTAÇÃO</small><h2>Artigo {piece.artigo}</h2><p>{piece.linha || 'Linha não informada'} · {piece.colecao || 'Coleção não informada'}</p></div><button onClick={() => setPresentationMode(false)} aria-label="Fechar modo apresentação">Fechar ×</button></header><main><section className="presentation-stage-media">{piece.desenho_tecnico_url ? <img src={piece.desenho_tecnico_url} alt={`Foto ou desenho técnico completo do artigo ${piece.artigo}`} /> : <div className="presentation-stage-empty">Sem foto ou desenho técnico cadastrado</div>}</section><aside><small>APONTAMENTO DA ANALISTA</small><p>{piece.observacoes || 'Nenhum apontamento registrado no cadastro.'}</p>{piece.pontos_atencao?.length > 0 && <section><b>Pontos de atenção</b><div className="attention-tags">{piece.pontos_atencao.map(tag => <span key={tag}>{tag}</span>)}</div></section>}</aside></main><footer><button onClick={() => selectRelative(-1)}>← Anterior</button><strong>{String(pieceIndex + 1).padStart(2, '0')} <small>de {String(presentationPieces.length).padStart(2, '0')}</small></strong><button onClick={() => selectRelative(1)}>Próximo →</button></footer></div>}
